...
MUTOOL = ".\\ExternalFiles\\mutool.exe"
...
def adjustPDF(contents):
  ...
  contents = contents[:startStartXref] + bytes("%i",'utf-8') % startXREF + contents[endStartXref:]
...
os.system(MUTOOL + ' merge -o merged.pdf .\\ExternalFiles\\dummy.pdf %s %s' % (sys.argv[1], sys.argv[2]))
...
contents = template % {bytes(k, 'utf-8'):v for k,v in locals().items()}
...
with open(".\\ExternalFiles\\dummy.pdf", "rb") as f:
  prefix1 = f.read()
...
with open(".\\ExternalFiles\\collision1.pdf", "wb") as f:
  f.write(file1)

with open(".\\ExternalFiles\\collision2.pdf", "wb") as f:
  f.write(file2)
